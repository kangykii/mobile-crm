...
export class BottomNav {
  ...
  renderSidebar(activeView) {
    return `
      <aside class="app-sidebar hidden lg:flex lg:w-56 lg:shrink-0 lg:flex-col lg:py-6">
        <div class="mb-6 px-3">
          <p class="text-xs font-bold uppercase tracking-[0.18em] text-[#969492]">Mini CRM</p>
          <p class="mt-1 text-sm font-semibold text-[#706E6B]">Web app</p>
        </div>
        <nav class="glass-nav flex flex-1 flex-col rounded-[1.5rem] p-3">
          <div class="glass-nav-inner flex flex-col gap-1.5">
            ${NAV_ITEMS.map((item) => this.itemTemplate(item, activeView, "sidebar")).join("")}
          </div>
          <div class="mt-auto border-t border-[rgba(60,60,67,0.08)] pt-3">
  ...
  itemTemplate(item, activeView, variant) {
    ...
    if (variant === "sidebar") {
      return `
        <button data-nav="${item.id}" class="inline-flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition ${activeClasses}" aria-label="${item.label}">
